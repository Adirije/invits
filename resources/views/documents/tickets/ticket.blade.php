...
<head>
    <meta charset="UTF-8">
    <title>{{ config('app.name') }} - {{ $event->name }}</title>
...
<body>
    <div class="container">
        @foreach ($transactions as $transaction)
            @foreach ($transaction->registrations as $i => $reg)
                <div class="section" style="margin-bottom: 50px">
                    <div class="left">
                        ...
                        <div>
                            <span class="seats" style="text-transform: uppercase; margin-right: 20px;"><strong>Type</strong><span>{{ $reg->ticket->name }}</span></span>
                        ...
                        <div class="info" style="margin-top: 10px;">
                            ...
                            {{ $reg->event->location->name }} <br> <span style="font-size: .7rem; padding-left:50px">{{ $reg->event->location->address }}</span>
                        </div>
                    </div>
                ...
                @if ($i +1 < count($transaction->registrations))
